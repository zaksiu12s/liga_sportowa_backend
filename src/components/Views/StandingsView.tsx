...
const StandingsView = () => {
  ...
  return (
    <div className="h-full max-w-4xl mx-auto flex flex-col py-6 px-4">
      <div className="flex flex-col items-center gap-6 flex-shrink-0 mb-12">
        ...
        <div className="flex space-x-6">
          {availableGroups.length > 0 ? (
            ...
            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Brak grup w bazie</span>
          )}
        </div>
      </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i}>
                  <td className="py-5 px-4"><Skeleton className="h-4 w-4" /></td>
                  <td className="py-5"><Skeleton className="h-4 w-48" /></td>
                  <td className="py-5"><Skeleton className="h-4 w-12 mx-auto" /></td>
                  <td className="py-5 px-4"><Skeleton className="h-4 w-8 ml-auto" /></td>
                </tr>
              ))
              ...
              <tr><td colSpan={4} className="py-20 text-center text-gray-400">BRAK DANYCH</td></tr>
            )}
          </tbody>
        </table>
      </div>
